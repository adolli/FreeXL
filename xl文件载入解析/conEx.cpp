...
// parser ////////////////////////////////////////////////////////
conExParser::conExParser(string exp):originExp(exp){
	parse(exp);
}
...
// conEx
// constructor
conEx::conEx(string exp,bool except):exceptThisConEx(except) {
	pConExGp=NULL;
...
bool conEx::match(TiXmlNode* node){
	...
	string value="";
	if (testAttrName!=""){
		if (!node->ToElement()) return false && !exceptThisConEx || true && exceptThisConEx;
		if (TIXML_SUCCESS!=node->ToElement()->QueryValueAttribute(testAttrName,&value)){
			return false && !exceptThisConEx || true && exceptThisConEx;
		}else if (testAttrValue!=""){
			if (testAttrMatchType=="!=" && value!=testAttrValue ||
				testAttrMatchType=="*=" && value.find(testAttrValue)!=string::npos ||
				testAttrMatchType=="^=" && value.find(testAttrValue)==0 ||
				testAttrMatchType=="$=" && value.find_last_of(testAttrValue)==value.length()-1 || //这一条要测试一下
				testAttrMatchType=="~=" && testAttrValue.find(value)!=string::npos ||
	...
	if (pConExGp->conexGp[0].filterExGp.empty()){
		...
		while(!stack.empty()){
			...
			if (tempNode->Type()!=$::XML_TEXT && pConExGp->gpMatch(tempNode)){
				// match到节点的处理部分
				matchCnt++;
				for (UINT i=0; i<filterExGp.size(); i++){//遍历filter，把满足filter的都集中起来
					if (filterExGp[i].filterType==conEx::FT_AT			&& filterExGp[i].filterValue==matchCnt || 
						filterExGp[i].filterType==conEx::FT_EVEN		&& matchCnt%2==0 ||
						filterExGp[i].filterType==conEx::FT_ODD			&& matchCnt%2==1 ||
						filterExGp[i].filterType==conEx::FT_FIRST		&& matchCnt==1
					){
						filterExGp[i]._used=true;
						ret.push_back(tempNode);
						break;//这个break表示或关系的filter,在匹配到一个之后可以不用再测试别的条件
					}else if (
						filterExGp[i].filterType==conEx::FT_NODETYPE	&& filterExGp[i].testNodeType==tempNode->Type() ||
						filterExGp[i].filterType==conEx::FT_NOT_AT		&& filterExGp[i].filterValue!=matchCnt
					){
			...
			for (TiXmlNode* child=tempNode->LastChild(); child; child=child->PreviousSibling()){
...
vector<TiXmlNode*> conEx::matchOnce(TiXmlNode* root){
	...
	vector<TiXmlNode*> ret;
	if (pConExGp->conexGp[0].filterExGp.empty()){
		for (TiXmlNode* loopNode = root->FirstChild(); loopNode; loopNode=loopNode->NextSibling()){
			// 遍历函数不遍历文本节点，访问文本节点请用.text()
			if(loopNode->Type()!=$::XML_TEXT && pConExGp->gpMatch(loopNode)){
				ret.push_back(loopNode);
			}	
			...
			if(loopNode->Type()!=$::XML_TEXT && pConExGp->gpMatch(loopNode)){
				...
				for (UINT i=0; i<filterExGp.size(); i++){//遍历filter，把满足filter的都集中起来
					if (filterExGp[i].filterType==conEx::FT_AT			&& filterExGp[i].filterValue==matchCnt || 
						...
						filterExGp[i].filterType==conEx::FT_NODETYPE	&& filterExGp[i].testNodeType==loopNode->Type() ||
						filterExGp[i].filterType==conEx::FT_NOT_AT		&& filterExGp[i].filterValue!=matchCnt
					){
						//在这一层判断里需要把所有match到的节点都判断一次，所以用过的filter不会被删除
						ret.push_back(loopNode);
...
bool conExGp::gpMatch(TiXmlNode* node){
	if (conexGp.empty()) return true;  //关于空节点到底属于false还是true？？还应该仔细考虑
	if (conexGp.size()==1){
...
void removeSpaceBeforeGT(string& conexp){
	...
	while (pos!=string::npos){
		i=0;
		while (pos-i-1>=0 && conexp[pos-i-1]==' ') i++;
		conexp.replace(conexp.begin()+pos-i,conexp.begin()+pos,"");
		pos=conexp.find('>',pos-i+1);
	}
}
char removeBraketPairFrom(string& conexp,string ls,string rs,string& lRemain,string& rRemain){
	char ret='-';
	int lPos=conexp.find(ls);
	int rPos=conexp.find_last_of(rs);
	if (lPos==string::npos || conexp.find(rs)==string::npos) return '-';// 因为find_last_of不测试完全匹配，所以要先用这个判断
	ret=lPos-1<0 ? '-' : conexp[lPos-1];
	if (rPos-rs.length()>=lPos+ls.length()){	
		lRemain=conexp.substr(0,lPos);
		rRemain=conexp.substr(rPos+1);
		conexp=conexp.substr(lPos+ls.length(),rPos-rs.length()-lPos-ls.length()+1);
	}else if (rPos==lPos+1){
