...
template<typename T>	vector<T> &			VecCombine(vector<T> &,const vector<T> &);//合并两个同类型的vector
...
public:
	...
	enum appendMethod{// append method
	...
	static	map<string,TiXmlDocument*> fileMap;									//文件名与文档节点的对应关系，相同文件在同一程序中只读取一次且xmlDoc相同
		...
		*/
			...
			$				  & appendElement(									//元素节点
									string tagName="",
									int method=$::APDM_ALL|$::APOS_INSIDE|$::APOS_AFTER|$::APDM_DOUBLETAG
								);
			$				  & appendText(										//文本节点
									string text="",
			...
			$				  & replaceWith(
									string withThis,
									int copyMethod=$::CM_ShallowCopy
								);												//用withThis作为选择器条件选出的节点替换当前选择器中的节点，可以一换多。
			$				  & replaceWith(
									$& withThis,								//用withThis中的节点替换selector中DOM节点
									int copyMethod=$::CM_ShallowCopy
								);												//将withThis代入selector中的节点
			$				  & replaceWithNewElement(string tagName);			//节点换成新元素，返回包含该元素的选择器
			$				  & clone(
									$& pasteTo,
									int position=$::APOS_AFTER,
									int copyMethod=$::CM_ShallowCopy
								);												//将节点复制到pasteTo之后或者之前的位置,注意！返回到选择器中的是新插入的节点
			$				  & cloneTo(
									$& pasteTo,
									int position=$::APOS_AFTER,
									int copyMethod=$::CM_ShallowCopy
								);												//与clone相同，但是选择器中的仍然是原来的节点
...
char removeBraketPairFrom(
	string& conexp,			//在匹配符号中间的表达式，即去除括号后中间的内容
	string ls,				//左括号
	string rs,				//右括号
